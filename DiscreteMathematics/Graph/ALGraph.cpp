...
template<class DataType>
bool ALGraph<DataType>::isEulerianGraph()
{
	for (int i = 0; i < vertexCount; i++)
	{
		Node* p = vertex[i].firstEdge;
		int count = 0;
		while (p)
		{
			count++;
...
template<class DataType>
void ALGraph<DataType>::fleury(int index)
{
	bool bridge = true;
	...
	{
		bridge = true;
		Node* p = vertex[stack[top]].firstEdge;
		while (p != NULL)
		{
			if (visited[stack[top]][p->index] == false)
			{
				bridge = false;
				visited[stack[top]][p->index] = true;
				break;
			}
			p = p->next;
		}
		if (bridge)
			cout << vertex[stack[top--]].data << ' ';
		else
			fleuryDFS(p->index, stack, &top, visited);
	}
}

template<class DataType>
void ALGraph<DataType>::fleuryDFS(int index, int stack[100], int *top, bool visited[100][100])
{
	stack[++*top] = index;
	Node* p = vertex[index].firstEdge;
	while (p != NULL)
	{
		if (visited[index][p->index] == false)
		{
			visited[index][p->index] = true;
			fleuryDFS(p->index, stack, top, visited);
			break;
		}
		p = p->next;
